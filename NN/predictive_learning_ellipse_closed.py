...
#教師データ（楕円形） 2*(教師データ数)の行列を作成
a=np.linspace(0,20,sample_no).reshape(sample_no,1) 
teacher=np.zeros(input_no*sample_no).reshape(sample_no,input_no)
teacher[:,0]=[0.8*np.cos(2.0*np.pi*i/20) for i in a]
teacher[:,1]=[0.8*np.sin(2.0*np.pi*i/20) for i in a]
...
#描画
plt.subplot(311)
plt.plot(yout[:,0],yout[:,1],"r",label="training data(open)")    #赤で学習結果を描く
plt.plot(teacher[:,0],teacher[:,1],"b",label="teaching data") #青で教師データを描く
plt.legend(loc="lower right")

plt.subplot(312)
plt.plot(yout_closed[:,0],yout_closed[:,1],"r",label="training data(closed)")    
plt.plot(teacher[:,0],teacher[:,1],"b",label="teaching data") 
plt.legend(loc="lower right")
