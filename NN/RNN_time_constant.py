...
#誤差
error=np.zeros((output_no)*N).reshape(N,output_no)
...
for loop in range(N):
    ...
    for t in range(1,sample_no-1):
        ...
        for i in range(hidden_no):
            ...
            u[t][i] = (1-1.0/t_cons)*u[t-1][i]+(1.0/t_cons)*(tmp1 + tmp2)
            h[t][i] = np.tanh(u[t][i])
        h[t][hidden_no] = 1.0
        ...
        for k in range(output_no):
            ...
            error[loop][k]+=(y[t][k]-ty[k])**2

    error[loop]=0.5*error[loop]    

    #バックプロップ
    for T in range(sample_no-2,0,-1):
        for i in range(hidden_no):
            tmp1 = 0.0
            tmp2 = 0.0
            for k in range(output_no):
                ...
                #print tmp1
            for id in range(hidden_no):
                tmp2+=w3[id][i]*h_back[T+1][id]
            h_back[T][i]=(tmp1+(1.0/t_cons)*tmp2)*(1-h[T][i]**2)+(1-1.0/t_cons)*h_back[T+1][i]
    ...
    for j in range(input_no+1):
        for i in range(hidden_no):
           w1[i][j]-=ETA*(1.0/t_cons)*d1[i][j]/(sample_no*input_no)
    for i in range(hidden_no+1):
        for k in range(output_no):
            w2[k][i]-=ETA*(1.0/t_cons)*d2[k][i]/(sample_no*input_no)
    for di in range(hidden_no+1):
        for i in range(hidden_no):
            w3[i][di]-=ETA*(1.0/t_cons)*d3[i][di]/(sample_no*input_no)
